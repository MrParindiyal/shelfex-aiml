...
with col1:
    ...
    text_input = st.text_area(
        "Generate customized Q/A flashcards from your own text for accelerated learning !",
...
def add_export_buttons(flashcards):
        ...
        for idx, card in enumerate(flashcards, start=1):
            writer.writerow(
                [
                    ...
                    card[
                        "question"
                    ],  # Note: lowercase 'q' and 'a' to match your format
                    card["answer"],
                ]
            )
...
if generate_button:
    if not text_input.strip():
        ...
        with st.spinner("Generating flashcards...", show_time=True):
            ...
            flashcards = generate_flashcards(
                text_input, difficulty, subject, num_flashcards
            )
        ...
        for j, col in enumerate(cols):
            if i + j < len(st.session_state.flashcards):
                ...
                with col:
                    ...
                    # Flip button
                    if st.button(
                        button_text, key=f"flip_{card_index}", use_container_width=True
                    ):
                        if card_index in st.session_state.flipped_cards:
    ...
    with col1:
        if st.button("Flip All Cards", use_container_width=True):
            if len(st.session_state.flipped_cards) == len(st.session_state.flashcards):
                ...
                st.session_state.flipped_cards = set(
                    range(len(st.session_state.flashcards))
                )
            st.rerun()
